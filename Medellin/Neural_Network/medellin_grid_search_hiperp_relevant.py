...
grid_result_sig = grid.fit(X_train_sig, y_train_sig,  validation_split=.1)


print("Melhores parâmetros encontrados com todas entradas: ", grid_result_sig.best_params_)
print("Melhor pontuação de MSE com todas entradas: ", grid_result_sig.best_score_)

means_sig = grid_result_sig.cv_results_['mean_test_score']
stds_sig = grid_result_sig.cv_results_['std_test_score']
params_sig = grid_result_sig.cv_results_['params']

all_data_sig = {}
counter = 0
for mean, stdev, param in zip(means_sig, stds_sig, params_sig):
    texto = "Mean: {}, stdev: {},  param: {}".format(mean, stdev, param)
    all_data_sig[counter] = texto
    counter += 1
    print(texto)

best_model_sig = grid_result_sig.best_estimator_
mse_test_sig = best_model_sig.score(X_test_sig, y_test_sig)

print("MSE: ", grid_result_sig.best_score_)

result_sig = best_model_sig.predict(X_test_sig)
print(result_sig)

result_sig = result_sig.reshape(232689,)

print_RMSE_MAE(y_test_sig, result_sig)

with open("best_model_relevant.json", "w") as outfile:
    json.dump(grid_result_sig.best_params_, outfile)

with open("all_data_relevant.json", "w") as outfile:
    json.dump(all_data_sig, outfile)
